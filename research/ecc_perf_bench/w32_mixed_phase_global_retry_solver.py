...
def solve(analysis: dict, budget_s: float, seed: int, catalog: dict, on_best=None) -> tuple[dict, dict]:
    ...
    if on_best is not None:on_best(best_state.copy(),{"objective_score":best,"objective_tuple":best_tuple,"event":"initial_feasible"})
            ...
            for net in order:
                ...
                if delta<=0 or rng.random()<math.exp(-delta/temp):
                    ...
                    if cand[0]<best or (cand[0]==best and cand[3]<best_tuple[3]):
                        ...
                        if on_best is not None:on_best(best_state.copy(),{"objective_score":best,"objective_tuple":best_tuple,"event":"improved"})
                else:
                    state[net]^=1
...
def run_solver_worker(payload):
    analysis,text,catalog,budget_s,seed,progress_root,worker_id=payload
    worker_dir=Path(progress_root)/f"worker-{worker_id}"
    worker_dir.mkdir(parents=True,exist_ok=True)
    def checkpoint(phase_bits,meta):
        candidate_text,target_specs=rewrite(text,analysis,phase_bits,catalog)
        cand_path=worker_dir/"best_candidate.v"
        tmp=cand_path.with_suffix(".v.tmp")
        tmp.write_text(candidate_text);os.replace(tmp,cand_path)
        record={"event":meta["event"],"objective_score":meta["objective_score"],"objective_tuple":meta["objective_tuple"],
          "candidate_graph_sha256":sha(cand_path),"phase_one_nets":sorted(n for n,v in phase_bits.items() if v),
          "changed_cells":target_specs,"solver_seed":seed,"worker_id":worker_id}
        out=worker_dir/"best_progress.json";tmpj=out.with_suffix(".json.tmp")
        tmpj.write_text(json.dumps(record,sort_keys=True,indent=2)+"\n");os.replace(tmpj,out)
    return solve(analysis,budget_s,seed,catalog,on_best=checkpoint)
...
def main():
    ...
    ap.add_argument("--solver-workers",type=int,default=2)
    ap.add_argument("--progress-dir",type=Path,default=None)
    args=ap.parse_args()
    ...
    actual_hostname=os.environ.get("CHIA_GCP_ACTUAL_HOSTNAME",container_hostname)
    resource_guard=None
    if args.mode=="solve":
        ...
        if not cpu_limit.exists() or not mem_limit.exists():
            raise ValueError("solver requires Docker cgroup v2 CPU and memory limits")
        quota,period=map(int,cpu_limit.read_text().split())
        memory_bytes=int(mem_limit.read_text().strip())
        if len(affinity)!=2 or quota/period>2.0 or memory_bytes>8*1024**3:
            raise ValueError(f"solver resource guard failed: affinity={affinity}, cpu_quota={quota}/{period}, memory_bytes={memory_bytes}")
        resource_guard={"actual_host_hostname_verified_by_wrapper":actual_hostname,"container_hostname":container_hostname,"cpu_affinity":affinity,"cpu_count":len(affinity),"cgroup_cpu_quota":quota,"cgroup_cpu_period":period,"cgroup_memory_limit_bytes":memory_bytes,"worker_count":args.solver_workers}
    ...
    if args.solver_workers!=2:
        raise ValueError("bounded W32 retry is fixed to exactly two workers")
    if args.progress_dir is None:
        raise ValueError("solve mode requires a per-worker progress directory")
    args.progress_dir.mkdir(parents=True,exist_ok=True)
    seeds=[args.seed,args.seed+1]
    payloads=[(analysis,text,catalog,args.budget_seconds,seeds[i],str(args.progress_dir),i+1) for i in range(2)]
    with concurrent.futures.ProcessPoolExecutor(max_workers=2) as pool:
        worker_results=list(pool.map(run_solver_worker,payloads))
    ranked=[]
    for idx,(phase_i,report_i) in enumerate(worker_results,1):
        ranked.append((100*report_i["best_xor3_count"]+report_i["best_xnor2_count"],report_i["best_total_xnor_count"],idx,phase_i,report_i))
    _,_,winner,phase,sol=min(ranked,key=lambda x:x[:3])
    sol["parallel_workers"]=2;sol["worker_seeds"]=seeds;sol["winning_worker"]=winner
    sol["all_worker_reports"]=[item[1] for item in worker_results]
